...
$skills = @(
    ...
    @{ id = "skill_voice_router_policy_v1"; name = "Voice Router Policy"; category = "voice"; file = "SKVOICE.WAS"; summary = "Selects text-first, direct-audio, or hybrid routing for bounded voice prompts without putting policy in the core."; rec = @("voice route", "text first", "direct audio", "hybrid voice") },
    @{ id = "skill_voice_audio_llm_bridge_v1"; name = "Voice Audio LLM Bridge"; category = "voice"; file = "SKVOICE.WAS"; summary = "Represents the sandbox-owned bridge for sending bounded compressed audio clips to a multimodal LLM when transcript-first is insufficient."; rec = @("direct audio", "multimodal llm", "audio reasoning", "voice prompt") }
)
...
$policies = @(
    ...
    @{ id = "policy_voice_privacy_v1"; scope = "voice"; summary = "Voice capture is user-controlled, bounded, visible, and transcript-confirmed before action."; rules = @("default off", "autonomy cannot enable microphone", "visible recording state required", "transcript before action by default", "bounded audio clips only", "direct audio only by route policy or explicit user request") },
    @{ id = "policy_voice_route_v1"; scope = "voice"; summary = "Voice routing defaults to text-first and escalates to direct audio only when justified."; rules = @("text-first is default", "hybrid preferred over always-direct audio", "direct-audio requires bounded compressed clips", "route policy belongs in sandbox skills", "core stores route state but does not decide semantic audio policy") },
    @{ id = "policy_platform_survival_core_v1"; scope = "platform"; summary = "New CPU and board ports may add only survival substrate to core; device behavior moves to sandbox capabilities."; rules = @("serial recovery before GUI", "CPU entry memory interrupts and recovery IO may be core", "virtio PCI USB display storage voice and policy remain capabilities", "query by arch and machine keys before generation", "upload platform evaluation after each boot milestone") }
)
