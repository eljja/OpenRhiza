    ...
    pub(crate) fn mark_allocated_region(&mut self, region: MemoryRegion) {
        for r in self.memory_map.iter_mut() {
            ...
            if r.region_type != MemoryRegionType::Usable {
                // Modern BIOSes (e.g. QEMU 10.x SeaBIOS) often mark sub-1MB conventional memory as Reserved.
                // Since the Bootloader, PageTable, and FrameZero reside here, overlaps are expected.
                // We just continue and let the split/overwrite logic claim the region.
                // Exception: if we are just marking FrameZero, we don't even need to overwrite the Reserved region.
                if region.region_type == MemoryRegionType::FrameZero {
                    return; 
                }
            }
        ...
        crate::printer::Printer.clear_screen();
        use core::fmt::Write;
        let _ = write!(crate::printer::Printer, "MAP DUMP:\n");
        for i in 0..self.memory_map.len() {
            let r = &self.memory_map[i];
            let _ = write!(crate::printer::Printer, "{:x?}..{:x?} {:?}\n", 
                r.range.start_frame_number * 4096, 
                r.range.end_frame_number * 4096, 
                r.region_type);
        }
